 ...
 */
public class CycSessionRequirementList<T extends UnsupportedCycOperationException>
        extends ArrayList<CycSessionRequirement<T>>
        implements List<CycSessionRequirement<T>>, CycSessionRequirement {
  ...
  @Override
  public CompatibilityResults checkCompatibility(CycSession session) 
    ...
    for (CycSessionRequirement requirement : this) {
      ...
      if (!reqResult.isCompatible()) {
        ...
        reqResult.getCompatibilityErrorMessages().forEach((errMsg) -> {
  ...
  public CompatibilityResults checkCompatibility() 
  ...
  public void throwExceptionIfIncompatible(CycSession session) 
          throws T, SessionCommunicationException, SessionCommandException {
    this.forEach((check) -> {
      check.throwExceptionIfIncompatible(session);
    });
  }
  
  public void throwExceptionIfIncompatible()
          throws T, SessionCommunicationException,
                    SessionCommandException,
                    SessionConfigurationException, 
                    SessionInitializationException {
