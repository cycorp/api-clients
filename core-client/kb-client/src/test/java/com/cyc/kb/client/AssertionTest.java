...
public class AssertionTest {
	...
	private static final Logger LOG = LoggerFactory.getLogger(AssertionTest.class.getName());
  ...
  public void testDelete() throws Exception {
    ...
    public void testRetriggerForwardInference () throws KbTypeException, CreateException, DeleteException, KbException {
      ...
      KbIndividual myFlying = KbIndividualFactory.findOrCreate("MyFlying", KbCollectionImpl.get("FlyingAnObject-Operate"));
      FactFactory.findOrCreate("(flyingDoneBySomething-Operate MyFlying WilliamHenryHarrison)", ctx.toString());
      Fact quotedIsaFact = FactFactory.get("(quotedIsa MyFlying ProprietaryTerm)", ctx.toString());
      assertTrue(quotedIsaFact instanceof Fact);
      ((FactImpl)quotedIsaFact).delete(true);
      Fact f = null;
      try {
        f = FactFactory.get("(quotedIsa MyFlying ProprietaryTerm)", ctx.toString());
      }catch (KbObjectNotFoundException e) {
        //we expect this to throw...
      }
      assertTrue(f ==  null);
      Fact doneByFact = FactFactory.get("(flyingDoneBySomething-Operate MyFlying WilliamHenryHarrison)", ctx.toString());
      ((FactImpl)doneByFact).retriggerForwardInference();
      assertTrue(FactFactory.get("(quotedIsa MyFlying ProprietaryTerm)", ctx.toString()) instanceof Fact);      
    }
