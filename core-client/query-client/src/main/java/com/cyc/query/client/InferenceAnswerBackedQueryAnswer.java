 ...
 * @author baxter
 */
public class InferenceAnswerBackedQueryAnswer implements ParaphrasedQueryAnswer {

  private final InferenceAnswer answerCyc;
  private final Paraphraser paraphraser;
  Map<Variable, Paraphrase> bindingParaphrases = new HashMap<>();
  ...
  public boolean hasBinding(Variable var) {
    try {
      return answerCyc.getBindings().containsKey((CycVariable) var.getCore()) && answerCyc.getBindings().get((CycVariable) var.getCore()) != null;
    } catch (CycConnectionException ex) {
      throw new RuntimeException(ex);
    }
  }
