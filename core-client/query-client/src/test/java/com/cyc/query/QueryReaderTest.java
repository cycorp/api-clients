...
public class QueryReaderTest {
  ...
  public void testLoadIstQuery() 
    ...
    final Query q1 = QueryFactory.getQuery(new SentenceImpl(BinaryPredicateImpl.get("ist"), TestUtils.INFERENCE_PSC,
                    TestUtils.xIsaBird()), TestUtils.INFERENCE_PSC);
    try {
  ...
  public void testLoadSortedQuery() 
    ...
    try {
      ...
      final KbIndividual queryObj = KbIndividualImpl.get("#$DemonstrateResultOrderingWithLengths-KBQ");
      final Query q = QueryFactory.getQuery(queryObj); // TODO: this test is throwing QueryConstructionExceptions
      System.out.println(q.getQuerySentence());
      ...
      for (QueryAnswer a : q.getAnswers()) {
        System.out.println("- " + a.getBinding(new VariableImpl("?ELT")));
      }
      assertTrue("Failed to get any answers.", q.getAnswerCount() > 0);
    } finally {
      q1.getId().delete();
  ...
  public void testQueryFromSimpleXML() 
    ...
    final Sentence expectedSentence = SentenceFactory.get(expectedSentenceStr);
    ...
    System.out.println("sort order: " + DefaultCycObject.cyclify(sortOrder));
