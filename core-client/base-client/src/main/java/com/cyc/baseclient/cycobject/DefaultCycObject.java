 ...
 *
 * @version $Id: DefaultCycObject.java 171043 2017-03-23 00:56:58Z nwinant $
 * @author  tbrussea
 *
 */
  ...
  public static Object fromPossibleCompactExternalId(String compactExternalId, CycAccess access)
    ...
    try {
      if (CompactHlidConverter.converter().isNumberCompactHLId(compactExternalId)) {
        ...
        CycList rawResult = access.converse().converseList("(multiple-value-list (maybe-find-object-by-compact-hl-external-id-string " + DefaultCycObject.stringApiValue(compactExternalId) + "))");
        if (((CycArrayList)rawResult).second() == CycObjectFactory.nil) {
          return null;
        } else {
