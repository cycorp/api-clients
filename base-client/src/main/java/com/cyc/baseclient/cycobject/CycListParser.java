...
public class CycListParser {
  ...
  //private final boolean dot = false;
  //private final boolean dotWord = false;
  //private final boolean dotParen = false;
  ...
  private static final int STEOF =  MyStreamTokenizer.TT_EOF;
 ...
 been parsed
  ...
  public String remainingString() {
    final StringBuilder remainingStringBuffer = new StringBuilder();
    if (tok == STEOF) {
      return "";
    }
    st.resetSyntax();
    ...
    try {
      while (true) {
        ...
        if ((readStack.sp > 0) && (parenLevel == 0)) {
          ...
          } else if (USE_NEW_REDUCE_DOTTED_PAIRS) {
  ...
  private void scanQuote() {
    ...
    if ((parenLevel > 0) && (parenLevel != readStack.sp)) {
      readStack.push(CONS_MARKER_SYMBOL);
    }
  ...
  private void scanRightParen() {
    ...
    while (readStack.sp > 2) {
      ...
      try {
        if ((readStack.peek()).equals(CONS_MARKER_SYMBOL)
            && (!firstElement.equals(CONS_MARKER_SYMBOL))
            && (!remainingElements.equals(CONS_MARKER_SYMBOL))) {
  ...
  private void checkQuotes() {
    if ((!quoteStack.empty())
        && (((Integer) quoteStack.peek()) == parenLevel)) {
      quoteStack.pop();
      endQuote = true;
    }
  }
