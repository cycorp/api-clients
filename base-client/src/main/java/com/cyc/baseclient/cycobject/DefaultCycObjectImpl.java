...
public abstract class DefaultCycObjectImpl implements CycObject {
  ...
  public static String cyclifyWithEscapeChars(Object obj, boolean isApi) {
    if (obj instanceof KbObject) {
      //return cyclifyWithEscapeChars(((KbObject) obj).getCore()); // TODO: is this the best place for this? - nwinant, 2017-08-29
      return ((KbObject) obj).stringApiValue(); // TODO: is this the best place for this? - nwinant, 2017-08-29
    }
    ...
    if (obj instanceof String) {
      ...
      if (CycStringUtils.is7BitASCII(str)) {
        return "\"" + CycStringUtils.escapeDoubleQuotes(str) + "\"";
      } else {
        return CycStringUtils.unicodeEscaped(str, isApi);
      }
    }
  ...
  public void toXML(XmlWriter xmlWriter, int indent, boolean relative)
          throws IOException {
    xmlWriter.printXMLStartTag(CYCL_OBJECT, indent, relative, true);
    xmlWriter.print(stringApiValue());
    xmlWriter.printXMLEndTag(CYCL_OBJECT, -indent, true);
  }
  
  @Deprecated
  protected void convertCycObjectToXML(CycObject cobj, XmlWriter xmlWriter, int indent, boolean relative)
          throws IOException {
    if ((cobj != null) && (cobj instanceof DefaultCycObjectImpl)) {
      ((DefaultCycObjectImpl) cobj).toXML(xmlWriter, indent, relative);
    }
  }
  ...
  public static String toCompactExternalId(Object cycObject, CycAccess access)
    ...
    if ((cycObject == null) || (!DefaultCycObjectImpl.isCycLObject(cycObject))) {
    ...
    Object result = CYC_OBJECT_TO_COMPACT_HL_EXTERNAL_ID_CACHE.get(cycObject);
    if (result != null) {
      return (String) result;
    }
    result = access.converse().converseObject("(compact-hl-external-id-string " + DefaultCycObjectImpl.stringApiValue(cycObject) + ")");
    if (!(result instanceof String)) {
      throw new BaseClientRuntimeException("Got invalid result: " + result);
    }
    CYC_OBJECT_TO_COMPACT_HL_EXTERNAL_ID_CACHE.put(cycObject, (String) result);
    return (String) result;
  }
  ...
  public static Object fromCompactExternalId(String compactExternalId, CycAccess access)
    ...
    Object result = COMPACT_EXTERNAL_ID_TO_CYC_OBJECT_CACHE.get(compactExternalId);
    if (result != null) {
      return result;
    }
    try {
      if (CompactHlidConverter.converter().isNumberCompactHLId(compactExternalId)) {
        ...
        result = access.converse().converseObject("(find-cycl-object-by-compact-hl-external-id-string " + DefaultCycObjectImpl.stringApiValue(compactExternalId) + ")");
      }
    } catch (IOException ioe) {
      throw new CycConnectionException(ioe);
    }
    COMPACT_EXTERNAL_ID_TO_CYC_OBJECT_CACHE.put(compactExternalId, result);
    return result;
  }
  ...
  public static Object fromPossibleCompactExternalId(String compactExternalId, CycAccess access)
          throws CycConnectionException {
    if (Objects.toString(compactExternalId, "").trim().isEmpty()) {
      throw new IllegalArgumentException(compactExternalId + " is not a valid compact external id.");
    }
    // TODO: support CompactHLIDConverter.converter() once we can simply identify
    // if a compact external id is a String or Number
    ...
    try {
      if (CompactHlidConverter.converter().isNumberCompactHLId(compactExternalId)) {
        ...
        final CycList rawResult = access.converse().converseList(
                "(multiple-value-list (maybe-find-object-by-compact-hl-external-id-string "
                        + DefaultCycObjectImpl.stringApiValue(compactExternalId) + "))");
        if (rawResult.second() == CycObjectFactory.nil) {
          return null;
        } else {
          result = rawResult.first();
        }
      }
    } catch (IOException ioe) {
      throw new CycConnectionException(ioe);
  ...
  private static final LinkedHashMap<String, Object> COMPACT_EXTERNAL_ID_TO_CYC_OBJECT_CACHE = new LinkedHashMap() {
  ...
  private static final LinkedHashMap<Object, String> CYC_OBJECT_TO_COMPACT_HL_EXTERNAL_ID_CACHE = new LinkedHashMap() {
