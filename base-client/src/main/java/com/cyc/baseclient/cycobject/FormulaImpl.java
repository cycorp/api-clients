...
public class FormulaImpl implements Formula {
  ...
  public List<CycVariable> findFreeVariables() {
    final List<CycVariable> freeVariables = new ArrayList<CycVariable>();
    gatherFreeVariables(freeVariables, new HashSet<CycVariable>(), false);
  ...
  public List<CycVariable> findQueryableVariables() {
    final List<CycVariable> queryableVariables = new ArrayList<CycVariable>();
    gatherFreeVariables(queryableVariables, new HashSet<CycVariable>(), true);
    return queryableVariables;
  }
    ...
    if (this instanceof FormulaSentenceImpl
      ...
      for (final Object arg : getArgsUnmodifiable()) {
        if (arg instanceof CycVariable) {
          ...
          ((FormulaImpl) arg).gatherFreeVariables(freeVarsSoFar, boundVars,
                  includeQueryable);
        }
      }
    }
  ...
  private void gatherQueryableVariablesFromConditionalSentence(Set<CycVariable> boundVars,
          List<CycVariable> freeVarsSoFar) {
    List<CycVariable> antecedentVars = new ArrayList<CycVariable>();
    final FormulaImpl antecedent = (FormulaImpl) getArg1();
    antecedent.gatherFreeVariables(antecedentVars, boundVars, true);
    for (final CycVariable antecedentVar : antecedentVars) {
      boundVars.add(antecedentVar);
      freeVarsSoFar.remove(antecedentVar);
    }
    final FormulaImpl consequent = (FormulaImpl) getArg2();
    consequent.gatherFreeVariables(freeVarsSoFar, boundVars, true);
    for (final CycVariable antecedentVar : antecedentVars) {
  ...
  private Set<CycVariable> gatherLocallyBoundVariables() {
    final Object arg1 = getArg1();
    final Set<CycVariable> locallyBoundVars = new HashSet();
    if (arg1 instanceof CycVariable) {
      locallyBoundVars.add((CycVariable) arg1);
    } else if (arg1 instanceof Formula) {
      for (final Object var : ((Formula) arg1).getArgs()) {
        if (var instanceof CycVariable) {
          locallyBoundVars.add((CycVariable) var);
        }
      }
    }
    return locallyBoundVars;
  }
