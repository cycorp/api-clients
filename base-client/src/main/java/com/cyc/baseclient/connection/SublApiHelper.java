...
public class SublApiHelper {
  ...
  private synchronized void recreateCycAccess() throws CycConnectionException {
    final CycAddress server = getCycAccess().getCycAddress();
    setCycAccess(CycClient.get(server));
    madeCycAccess = true;
  }
  ...
  public static String wrapVariableBinding(String command, CycSymbol variable, Object value) {
    try {
      ...
      return "(clet ((" + DefaultCycObjectImpl.cyclifyWithEscapeChars(variable, true)
              + " " + getAPIString(value) + ")) " + command + ")";
    } catch (Exception e) {
      return command;
    }
  }
  ...
  public static String makeSublStmt(CycSymbol function, Object... params) {
    return SublApiHelper.makeSublStmt(function.getSymbolName(), params);
  }
  ...
  public static AsIsTerm makeNestedSublStmt(String functionName, Object... params) {
    return new AsIsTerm(SublApiHelper.makeSublStmt(functionName, params));
  }
