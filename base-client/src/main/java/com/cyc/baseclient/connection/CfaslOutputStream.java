...
public class CfaslOutputStream
  ...
  public void writeString(String s)
    ...
    if (trace == API_TRACE_DETAILED) {
      ...
      if (CycStringUtils.is7BitASCII(escapedString)) {
          write(CFASL_STRING);
      } else {
 ...
 list.
  ...
  public void writeResultSetSlice(ResultSetSlice rss) throws IOException {
    ...
    List<List<Object>> sliceRows = rss.getSliceRows();
  
    write(CFASL_RESULT_SET_SLICE);
    writeInt(rss.sliceRowCount());
    writeInt(rss.getColumnCount());
    writeInt(rss.first());
    writeBoolean(rss.hasNext());
  
    try {
      for (List<Object> row : sliceRows) {
        for (Object columnValue : row) {
          if (columnValue instanceof Array) {
            Array arr = (Array) columnValue;
            Object[] valArr = (Object[]) arr.getArray();
            writeObject(valArr);
          } else {
            writeObject(columnValue);
          }
        }
      }
    }
