...
class TaskProcessorBinaryResponseHandler extends Thread {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(TaskProcessorBinaryResponseHandler.class);
  ...
  public void run() {
    ...
    try {
      ...
      while ((!isClosed) && (!isClosing)) {
        ...
        try {
          ...
          if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("API response: " + taskProcessorResponse.stringApiValue());
          }
        ...
        LOGGER.trace("API status: " + status);
        ...
        try {
          ...
          final Object response = CycStringUtils.cyclStringsToJavaStrings(
          ...
          final Runnable notificationTask = new NotificationTaskImpl(taskStatus, objectIsInvalid, worker, response, finished, id, cycConnection);
          try {
      ...
      if (closingException != null) {
        LOGGER.error("TaskProcessor terminated because of exception.", closingException);
      }
