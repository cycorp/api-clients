...
public class OwlToolImpl extends AbstractKbTool {
  ...
  public OwlToolImpl(CycAccess client) {
    super(client);
  }
   ...
   * @param ontology the existing FortImpl representing the ontology, if we are to update an
   * already-imported ontology
   *
   * @return the FortImpl representing the imported ontology
   *
   * @throws CycConnectionException if a communications error occurs
   ...
   * @param ontology the existing FortImpl representing the ontology, if we are to update an
   *                 already-imported ontology
   * @param quotedIsa the collection of which newly created terms should be quoted instances. 
   *                  If null, Cyc will create one.
   *
   * @return the FortImpl representing the imported ontology
   *
   * @throws CycConnectionException if a communications error occurs
  ...
  public Fort importOwlOntology(final String uri, final String prefix,
          ...
          final Fort cyclist)
          throws CycConnectionException, CycApiException {
    final String sourceFileString = ((sourceFile != null) && (sourceFile.length() > 0))
            ? "\"" + sourceFile + "\""
            : "nil";
    final String ontologyString  = (ontology == null)  ? "nil" : FortImpl.stringApiValue(ontology);
    final String quotedIsaString = (quotedIsa == null) ? "nil" : FortImpl.stringApiValue(quotedIsa);
    final String cyclistString   = (cyclist == null)   ? "nil" : FortImpl.stringApiValue(cyclist);
    final Object object = getConverse().converseObject(
            "(import-owl-ontology " + "\"" + uri + "\" " + "\"" + prefix + "\" "
            + sourceFileString + " " + ontologyString
            + " " + quotedIsaString + " " + cyclistString + ")");
    return (object instanceof Fort) 
            ? (Fort) object
            : null;
  }
}
