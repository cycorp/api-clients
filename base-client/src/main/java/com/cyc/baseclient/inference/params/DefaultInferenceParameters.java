...
import static com.cyc.query.parameters.InferenceParameters.ABDUCTION_ALLOWED;
...
public class DefaultInferenceParameters extends SpecifiedInferenceParameters {
  ...
  private static CycList getParams(CycAccess cyc, String params) throws CycConnectionException {
    final CycList value;
    if (PARAM_CACHE.containsKey(params)) {
      value = PARAM_CACHE.get(params);
    } else {
      CycList paramList = cyc.converse().converseList("'(" + params + ")");
      PARAM_CACHE.put(params, paramList);
      value = paramList;
    }
    if (value != null) {
  ...
  private static final Map<String, CycList> PARAM_CACHE = new ConcurrentHashMap<String, CycList>();
  ...
  @Override
  public DefaultInferenceParameters setMaxAnswerCount(final Integer max) {
    put(MAX_NUMBER, max);
    return this;
  }

  @Override
  public DefaultInferenceParameters setMaxTransformationDepth(Integer max) {
  ...
  public Object put(String parameterName, Object value) {
    ...
    if (value instanceof CycSymbol && ((CycSymbol) value).toString().equals(":UNKNOWN")) {
      ...
      if (value instanceof CycArrayList || value instanceof CycSymbol || value instanceof Integer) {
        ...
        throw new BaseClientRuntimeException("Got invalid value " + value 
  ...
  private InferenceParameterDescriptions getDefaultInferenceParameterDescriptions() {
    if (defaultInferenceParameterDescriptions == null) {
      defaultInferenceParameterDescriptions = DefaultInferenceParameterDescriptions
  ...
  public static void main(String[] args) {
    try {
      ...
      parameters.put(":MAX-NUMBER", 10);
      parameters.put(":PROBABLY-APPROXIMATELY-DONE", 0.5);
      parameters.put(":ABDUCTION-ALLOWED?", true);
