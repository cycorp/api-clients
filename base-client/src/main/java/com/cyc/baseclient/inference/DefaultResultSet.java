 ...
 * <code><pre>
 ...
 try {
   ...
   DefaultResultSet rs = worker.executeQuerySynch();
   try {
 ...
 * <p>
 * @author tbrussea
 * @since Mar 22, 2010, 11:55 AM
 * @version $Id: DefaultResultSet.java 176591 2018-01-09 17:27:27Z nwinant $
 */
...
public final class DefaultResultSet extends AbstractResultSet {
 ...
 <code>Naut</code>
   ...
   * column identified by <code>col</code> which should be a <code>CycVariableImpl</code> in the
   * original query. Returns <code>null</code> if no value is set for the current row and given
   * column.
   *
   * <p>
   ...
   *
   * <p>
   * NOTE: Fails on dates that are not CycNauts using #$YearFn (i.e. it will not work on skolemized
   * dates, or other forms of dates that don't use the #$YearFn vocabulary).
   *
   * @param col the name of the variable that represents the column of interest
   * @return the value, as a <code>java.util.Date</code>, at the current row and at the column
   * identified by the <code>col</code> which is a <code>CycVariableImpl</code> in the original
   * query. Returns <code>null</code>, if no value is set for the current row and given column.
   * @throws CycApiException if called on a closed result set
   * @throws IllegalArgumentException if <code>col</code> is not valid
   * @throws ArrayIndexOutOfBoundsException if the current cursor is not on a valid row
   * @throws ClassCastException if the value is not convertible to a
  ...
  public <T> T getObject(int columnIndex, Class<T> type) throws SQLException {
    throw new UnsupportedOperationException("Not supported yet."); // FIXME: implement
  }
