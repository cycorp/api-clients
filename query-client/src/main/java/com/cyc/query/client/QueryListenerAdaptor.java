...
import com.cyc.query.client.QueryInference.QueryWorker;
...
public class QueryListenerAdaptor implements InferenceWorkerListener {
  ...
  public void notifyInferenceAnswersAvailable(InferenceWorker inferenceWorker, List newAnswers) {
    final List<QueryAnswer> newQueryAnswers = new ArrayList<>(newAnswers.size());
    final QueryImpl query = getQuery(inferenceWorker);
    for (final Object newAnswer : newAnswers) {
      try {
        final List<CycList> cycBindings
                = CycObjectFactory.nil.equals(newAnswer) ? Collections.<CycList>emptyList() : (List<CycList>) newAnswer;
        newQueryAnswers.add(getQueryAnswerFromCycBindings(query, cycBindings));
      } catch (Exception ex) {
        ex.printStackTrace();
      }
    }
  ...
  // Private
  private QueryAnswer getQueryAnswerFromCycBindings(final QueryImpl query, final List<CycList> cycBindings)
   ...
   * @return
   */
  private Map<Variable, Object> gatherBindings(List<CycList> cycBindings,
          final QueryAnswer qa) {
    final Map<Variable, Object> bindings = new HashMap<>(cycBindings.size());
    boolean qaGood = true;
    for (final CycList cycBinding : cycBindings) {
      try {
        final VariableImpl var = new VariableImpl((CycVariable) cycBinding.first());
        final Object val = KbObjectImpl.checkAndCastObject(cycBinding.rest());
