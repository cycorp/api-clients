...
public class QueryReader {
  ...
  public Query convertQuery(CyclQuery cyclQuery) throws KbException, QueryConstructionException {
    ...
    final Sentence querySentence = Sentence.get(convertFormula(cyclQuery.getQueryFormula()));
    ...
    final QueryImpl query = (QueryImpl) Query.get(querySentence, queryContext, queryParams);
  ...
  private Context convertMt(QueryMt queryMt) throws KbException {
    if (queryMt.getConstant() != null) {
      return Context.get(((CycConstant) translateObject(
      ...
      final ElMt elmt = (mtNAT instanceof Nart) //@todo mtNat can probably be sent directly to Context.get(), without trying to convert to either an ELMt or a CycNaut
              ? ElMtNartImpl.makeElMtNart((Nart) mtNAT)
...
//  }
  ...
  private void warnOrThrowException(boolean shouldWarn, String errorMsg, String warnConsequenceMsg) {
    if (shouldWarn) {
      LOG.error(errorMsg + warnConsequenceMsg);
    } else {
  ...
  private InferenceParameters convertParams (
    ...
    final String paramString = sb.toString();
    LOG.debug("InferenceParameters string: \"{}\"", paramString);
    return new DefaultInferenceParameters(CycAccessManager.getCurrentAccess(), paramString);
  }
  ...
  private String processXml(String rawXml) {
    ...
    if (hasMungedLists) {
      ...
      LOG.debug("MUNGED XML! Working around...");
      ...
      if (LOG.isTraceEnabled()) {
        ...
        LOG.trace(
                ...
                result);
        LOG.trace("{} instances of {}", count(result, LIST_BEGIN_TAG), LIST_BEGIN_TAG);
        LOG.trace("{} instances of {}", count(result, LIST_CLOSE_TAG), LIST_CLOSE_TAG);
        LOG.trace("{} instances of {}", count(result, TEMP_PRED_NAME), TEMP_PRED_NAME);
        LOG.trace("{} instances of {}", count(result, SENT_BEGIN_TAG), SENT_BEGIN_TAG);
        LOG.trace("{} instances of {}", count(result, SENT_CLOSE_TAG), SENT_CLOSE_TAG);
      }
    } else {
      result = rawXml;
    }
