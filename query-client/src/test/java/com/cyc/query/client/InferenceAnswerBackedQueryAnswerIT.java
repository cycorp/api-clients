...
public class InferenceAnswerBackedQueryAnswerIT {
  ...
  public void testGetBinding() throws KbTypeException, QueryConstructionException, SessionCommunicationException {
    ...
    final Variable var = Variable.get("?SENT");
    final List<QueryAnswer> a = Query.get("(equals ?SENT (likesAsFriend GeorgeWashington JohnAdams))").getAnswers();
  ...
  public void testGetSources() throws KbTypeException, CreateException, SessionCommunicationException, QueryConstructionException, KbException {
    ...
    assumeNotOpenCyc();
    final Context WHH_WP_CTX = Context.findOrCreate("(#$ContextOfPCWFn #$TestFactEntrySource-WikipediaArticle-WilliamHenryHarrison)");
    final Sentence querySentence = Sentence.get("(#$isa #$WilliamHenryHarrison (#$FormerFn #$UnitedStatesPresident))");
    final Query q = Query.get(querySentence, WHH_WP_CTX);
    q.retainInference();
